...
def fair2_project_climate(
    ...
    fill(
        ...
        * df_configs["fscale_Volcanic"].values.squeeze(),
        specie="Volcanic",
    )
    ...
    fill(
        ...
        * df_configs["fscale_solar_amplitude"].values.squeeze()
        + trend_shape[:, None, None]
        * df_configs["fscale_solar_trend"].values.squeeze(),
        specie="Solar",
    )
    ...
    # carbon cycle
    fill(
        f.species_configs["iirf_0"], df_configs["cc_r0"].values.squeeze(), specie="CO2"
    )
    fill(
    ...
    attrs = {
        ...
        "Method": ("Temperature retunr from f = FAIR(ch4_method='Thornhill2021')."),
    ...
    tempds = xr.Dataset(
        ...
        coords={"years": proj_years, "locations": [-1], "samples": np.arange(nsamps)},
        attrs=attrs,
    )

    deeptempds = xr.Dataset(
        {
            "deep_ocean_temperature": (
                ("samples", "years", "layers", "locations"),
                deeptemps,
                {"units": "degC"},
            ),
            "lat": (("locations"), [np.inf]),
    ...
    pooledds = xr.Dataset(
        ...
        coords={"years": proj_years, "samples": np.arange(nsamps), "layers": [1, 2]},
        attrs=attrs,
    )
...
def CenterSmooth(temp, years, cyear_start, cyear_end, smooth_win):
    ...
    if smooth_win > 1:
        csmooth_temp = np.apply_along_axis(
            Smooth, axis=1, arr=center_temp, w=smooth_win
        )
    else:
